...
namespace CMZServerHost
{
    ...
    public sealed class ServerWorldHandler
    {
        ...
        private readonly Dictionary<string, int[]> _chunkCache = new Dictionary<string, int[]>();

        /// <summary>LRU order for chunk cache entries.</summary>
        private readonly LinkedList<string> _chunkCacheOrder = new LinkedList<string>();

        /// <summary>Lookup for LRU linked-list nodes.</summary>
        private readonly Dictionary<string, LinkedListNode<string>> _chunkCacheNodes = new Dictionary<string, LinkedListNode<string>>();
        ...
        /// </summary>
        private readonly HashSet<byte> _chunkRequestLoggedForPlayer = new HashSet<byte>();
        ...
        public void Init(Assembly gameAsm, Assembly commonAsm)
        {
            ...
            _messageIdToType = new Dictionary<byte, string>();
            _typeToMessageId = new Dictionary<string, byte>();
            ...
            try
            {
                ...
                if (registerMethod != null)
                {
                    try
                    {
                        registerMethod.Invoke(null, new object[] { _gameAsm, _commonAsm });
                        registerMethod.Invoke(null, new object[] { _commonAsm, _gameAsm });
                    }
                    catch (Exception ex)
                    {
                ...
                var types = typesField?.GetValue(null) as Type[];
                var ids = idsField?.GetValue(null) as System.Collections.IDictionary;
        ...
        /// <summary>
        /// Loads the current world's <c>world.info</c> file through the reflected save device.
        ///
        /// Notes:
            ...
            try
            {
                ...
                bool ok = InvokeSaveDeviceLoad(relativePath, stream =>
                {
                    using (var reader = new BinaryReader(stream))
                    {
                        ...
                        if (defaultCtor == null)
                            throw new InvalidOperationException("WorldInfo constructors not found.");
                        ...
                        if (loadMethod == null)
                            throw new InvalidOperationException("WorldInfo.Load(BinaryReader) not found.");
        ...
        ///
        /// Return behavior:
        /// - <c>true</c>: the message was consumed by the host and should not be relayed.
        /// - <c>false</c>: the message was not consumed and the outer network layer may relay it normally.
        ///
        /// Notes:
            ...
            Action<object, byte[], byte> sendToClient)
        {
            ...
            {
                HandlePlayerExistsMessage(senderId, senderConn, data);
                return false;
            }
        ...
        private void HandleRequestWorldInfo(byte senderId, object senderConn, Action<object, byte[], byte> sendToClient)
        {
            ...
            try
            {
                ...
                using (var writer = new BinaryWriter(ms))
                {
                    ...
                    var saveMethod = _worldInfo.GetType().GetMethod("Save", new[] { typeof(BinaryWriter) });
                    saveMethod?.Invoke(_worldInfo, new object[] { writer });
        ...
        private void SaveRawInventoryBytesForGamer(object gamer, byte[] invBytes)
        {
            ...
            if (!string.IsNullOrWhiteSpace(dir) && !_saveDeviceDirectoryExists(dir))
            {
                _saveDeviceCreateDirectory(dir);
            }
        ...
        private bool InvokeSaveDeviceSave(string relativePath, Action<Stream> callback)
        {
            ...
            try
            {
                ...
                saveMethod.Invoke(_saveDevice, new object[] { relativePath, true, true, fileAction });
                return true;
            }
            catch (Exception ex)
            {
        ...
        /// </summary>
        private bool _saveDeviceDirectoryExists(string relativePath)
        {
            try
            {
                MethodInfo mi = _saveDevice.GetType().GetMethod("DirectoryExists", new[] { typeof(string) });
                if (mi == null)
                    return false;

                return (bool)mi.Invoke(_saveDevice, new object[] { relativePath });
            }
            catch
            {
                return false;
        ...
        /// </summary>
        private void _saveDeviceCreateDirectory(string relativePath)
        {
            try
            {
                MethodInfo mi = _saveDevice.GetType().GetMethod("CreateDirectory", new[] { typeof(string) });
                mi?.Invoke(_saveDevice, new object[] { relativePath });
            }
            catch
            {
            }
        ...
        private string ComputeVanillaHashString(string value)
        {
            ...
            if (md5Type == null)
                throw new InvalidOperationException("MD5HashProvider type not found.");
            ...
            if (computeMethod == null)
                throw new InvalidOperationException("MD5HashProvider.Compute(byte[]) not found.");

            object hash = computeMethod.Invoke(md5, new object[] { Encoding.UTF8.GetBytes(value ?? string.Empty) });
            if (hash == null)
                throw new InvalidOperationException("MD5 hash result was null.");
        ...
        /// </summary>
        private void HandlePlayerExistsMessage(byte senderId, object senderConn, byte[] data)
        {
            try
            {
        ...
        private void HandleItemCrateMessage(byte senderId, byte[] data)
        {
            try
            {
                ...
                applyMethod.Invoke(msg, new object[] { _worldInfo });
        ...
        private void HandleRemoveBlocksMessage(byte senderId, byte[] data)
        {
            try
            {
                ...
                Array blocks = GetMemberValue(msg, "BlocksToRemove") as Array;
        ...
        private void SaveWorldInfo()
        {
            ...
            saveMethod.Invoke(_worldInfo, new object[] { null, _saveDevice });
        }
        ...
        private object DeserializeGameMessage(string fullTypeName, byte[] packetData)
        {
            ...
            Type msgType = ResolveType(fullTypeName);
            if (msgType == null)
                throw new InvalidOperationException("Message type not found: " + fullTypeName);
            ...
            if (ctor == null)
                throw new InvalidOperationException("Default ctor not found for " + fullTypeName);
            ...
            if (recv == null)
                throw new InvalidOperationException("RecieveData(BinaryReader) not found for " + fullTypeName);

            // packet layout: [messageId][payload...][checksum]
            using (var ms = new MemoryStream(packetData, 1, packetData.Length - 2, false))
            using (var reader = new BinaryReader(ms))
            {
                recv.Invoke(msg, new object[] { reader });
            }
        ...
        private bool DictionaryRemove(object dictionary, object key)
        {
            ...
            MethodInfo mi = dictionary.GetType().GetMethod("Remove", new[] { key.GetType() });
            if (mi == null)
                return false;

            object result = mi.Invoke(dictionary, new[] { key });
        ...
        private void InitSaveDevice()
        {
            try
            {
                ...
                object hashObj = computeMethod.Invoke(md5, new object[] { sourceBytes });
                if (hashObj == null)
                {
                ...
                byte[] key = dataProp.GetValue(hashObj, null) as byte[];
                if (key == null)
                {
                ...
                _saveDevice = Activator.CreateInstance(fsSaveType, new object[] { _saveRoot, key });
        ...
        private bool InvokeSaveDeviceLoad(string relativePath, Action<Stream> callback)
        {
            ...
            try
            {
                ...
                loadMethod.Invoke(_saveDevice, new object[] { relativePath, fileAction });
                return true;
            }
            catch
            {
