...
namespace CMZServerHost
{
    ...
    internal sealed class ServerRuntime
    {
        ...
        /// </summary>
        public ServerRuntime(ServerConfig config, Assembly gameAsm, Assembly commonAsm)
        {
            _config = config;
            _gameAsm = gameAsm;
        ...
        private void CreateSyntheticHostGamer()
        {
            ...
            object playerId = Activator.CreateInstance(playerIdType, new object[] { hostHash });

            object signedInGamer = Activator.CreateInstance(
                signedInGamerType,
                new object[] { playerIndexOne, playerId, "Server" });

            // session = null for now, local = true, host = true, globalID = 0
            _hostGamer = Activator.CreateInstance(
                networkGamerType,
                new object[] { signedInGamer, null, true, true, (byte)0 });
        }
