...
namespace CMZServerHost
{
    ...
    public sealed class CmzMessageCodec
    {
        ...
        private readonly Dictionary<byte, string> _messageIdToType = new Dictionary<byte, string>();
        ...
        {
            var messageType = ResolveType("DNA.Net.Message");
            if (messageType == null)
                throw new InvalidOperationException("DNA.Net.Message type not found.");
            ...
            var ids = idsField?.GetValue(null) as IDictionary;
        ...
        public byte[] BuildPayload(string fullTypeName, Action<object> initMessage)
        {
            ...
            var msgType = ResolveType(fullTypeName);
            if (msgType == null)
                throw new InvalidOperationException("CLR type not found: " + fullTypeName);

            object msg = CreateMessageInstance(msgType);
            if (msg == null)
                throw new InvalidOperationException("Failed to create message instance: " + fullTypeName);

            initMessage?.Invoke(msg);

            var sendData = FindInstanceMethod(msgType, "SendData", new[] { typeof(BinaryWriter) });
            if (sendData == null)
                throw new InvalidOperationException(fullTypeName + ".SendData(BinaryWriter) not found.");
            ...
            using (var writer = new BinaryWriter(ms))
            {
                ...
                sendData.Invoke(msg, new object[] { writer });
                writer.Flush();
        ...
        public void SetEnumMember(object instance, string memberName, string enumTypeName, object rawValue)
        {
            ...
            if (enumType == null)
                throw new InvalidOperationException("Enum type not found: " + enumTypeName);
